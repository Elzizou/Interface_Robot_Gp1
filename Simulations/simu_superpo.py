...
def main(surface):
    ...
    while running:
        ...
        if indice < taille:
            ...
            fin_commandes= False
            print("\n######### Début des commandes ###########\n")
            x_ephemere, y_ephemere, theta_ephemere = x, y, theta
            nombre_ordres=0
            while not fin_commandes:
                commande_boolean=input("Voulez-vous ajouter une commmande ? ('O' pour oui et 'N' pour non): ")
                if commande_boolean == "O" or commande_boolean == "o":
                    new_commande = False
                    while not new_commande:
                        commande=input("Veuillez sélectionner une commande (L, R ou A) ou rentrez E pour arreter la simulation: ")
                        if commande == "L":
                            new_commande = True
                            nombre_ordres += 1
                            distance = float(input("Rentrez la longueur de la ligne: "))
                            x_ephemere += distance * numpy.cos(theta)
                            y_ephemere += distance * numpy.sin(theta)
                            if nombre_ordres ==1:
                                vitesses_droit, vitesses_gauche, tau_p = ligne_theo(distance)
                            else:
                                vitesses_droit_futur, vitesses_gauche_futur, tau_f = ligne_theo(distance)

                        elif commande == "R":
                            new_commande = True
                            nombre_ordres += 1
                            angle = float(input("Rentrez la valeur de l'angle de rotation, en rad: "))
                            theta_ephemere += angle
                            if nombre_ordres ==1:
                                vitesses_droit, vitesses_gauche, tau_p = rot_theo(angle)
                            else:
                                vitesses_droit_futur, vitesses_gauche_futur, tau_f = rot_theo(angle)
                            ...
                            if nombre_ordres ==1:
                                vitesses_droit, vitesses_gauche, tau_p = arc_theo(x_ephemere, y_ephemere, theta_ephemere, xc, yc)
                            else:
                                vitesses_droit_futur, vitesses_gauche_futur, tau_f = arc_theo(x_ephemere, y_ephemere, theta_ephemere, xc, yc)
                            x_ephemere, y_ephemere = xc, yc

                        else:
                            print(" !! Commande erronée !! ")

                        if (nombre_ordres > 1 and new_commande):
                            if tau_p >= tau_f:
                                vitesses_droit = vitesses_droit[0:len(vitesses_droit) - int(tau_f / delay)] + list(
                                    map(add, vitesses_droit[len(vitesses_droit) - 1 - int(tau_f / delay):],
                                        vitesses_droit_futur[0:int(tau_f / delay)]))
                                vitesses_gauche = vitesses_gauche[0:len(vitesses_gauche) - int(tau_f / delay)] + list(
                                    map(add, vitesses_gauche[len(vitesses_gauche) - 1 - int(tau_f / delay):],
                                        vitesses_gauche_futur[0:int(tau_f / delay)]))
                            else:
                                vitesses_droit = vitesses_droit[0:len(vitesses_droit) - int(tau_p / delay)] + list(
                                    map(add, vitesses_droit[len(vitesses_droit) - int(tau_p / delay):],
                                        vitesses_droit_futur[0:int(tau_p / delay)]))
                                vitesses_gauche = vitesses_gauche[0:len(vitesses_gauche) - int(tau_p / delay)] + list(
                                    map(add, vitesses_gauche[len(vitesses_gauche) - int(tau_p / delay):],
                                        vitesses_gauche_futur[0:int(tau_p / delay)]))

                            vitesses_droit = vitesses_droit + vitesses_droit_futur[int(tau_f / delay):]
                            vitesses_gauche = vitesses_gauche + vitesses_gauche_futur[int(tau_f / delay):]

                            tau_p = tau_f

                else:
                    print("\n######## Fin des commandes ##########\n")
                    taille = len(vitesses_droit)
                    maximum = max(max(vitesses_droit), max(vitesses_gauche))
                    fin_commandes=True
                    if (nombre_ordres==0):
                        starting=False
