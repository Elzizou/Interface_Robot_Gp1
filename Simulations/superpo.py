...
def main():
    ...
    for compteur in range(len(ordres) - 1):
        ...
        if (commande_p != "R" and commande != "R"):

            tau_min = min(tau_p, tau_f)
            v_droit = v_droit[0:len(v_droit)-int(tau_min/delay)] + list(map(add, v_droit[(len(v_droit) - int(tau_min/delay)):], future_droit[0:int(tau_min/delay)]))
            v_gauche = v_gauche[0:len(v_gauche) - int(tau_min / delay)] + list(map(add, v_gauche[(len(v_gauche)- int(tau_min / delay)):], future_gauche[0:int(tau_min / delay)]))


            v_droit = v_droit + future_droit[int(tau_min/delay):]
            v_gauche = v_gauche + future_gauche[int(tau_min / delay):]

        else:
            v_droit = v_droit + future_droit
            v_gauche = v_gauche + future_gauche
