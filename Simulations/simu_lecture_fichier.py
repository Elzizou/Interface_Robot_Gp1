...
def drawGraph(surface, taille, maximum):
    ...
    for indice in range(len(lines_vitesses_gauche)):
        ...
        pygame.draw.line(surface, colors["blue"],
                         (origine_graph[0] + indice * scale_10ms, origine_graph[1] - line[0] * scale_rad),
...
def main(surface):
    ...
    while running:
        ...
        if indice < taille:
            ...
            compteur += 1
            if compteur < len(ordres):

                commande = ordres[compteur].split(",")[0]
                if commande == "L":
                    distance = float(ordres[compteur].split(",")[1])
                    ligne_theo(distance)
                    taille = len(vitesses_droite)
                    maximum = max(max(vitesses_droite), max(vitesses_gauche))

                elif commande == "R":
                    angle = float(ordres[compteur].split(",")[1])
                    rot_theo(angle)
                    taille = len(vitesses_droite)
                    maximum = max(max(vitesses_droite), max(vitesses_gauche))

                elif commande == "A":
                    xc = float(ordres[compteur].split(",")[1])
                    yc = float(ordres[compteur].split(",")[2])
