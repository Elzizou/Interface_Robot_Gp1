...
try:
    ...
    print("Speech recognition Library needed\nHere is the error : ", e)
...
try:
    ...
    print("Bezier Library needed \nHere is the error: ", e)

robot_path = os.path.join(os.path.dirname(__file__), 'robot.png')
# print(robot_path)
command_path = os.path.join(os.path.dirname(__file__), "values.txt")
...
class Interface():
    ...
    def __init__(self):
        ...
        self.robot_im = plt.imread(robot_path)
        self.imgplot = self.ax.imshow(self.robot_im, origin='lower', extent=([-1, 1, -1, 1]))
    ...
    def set_buttons(self):
        ...
        self.precision_scale = tkinter.Scale(self.query_frame, from_=1, to=20, orient="horizontal", tickinterval=2,
                                             length=100, label='Precision')
    ...
    def on_key_press(self, event):
        ...
        # Update Canvas
        self.update_graph()
        self.press = self.plt_draw[-1][0], self.plt_draw[-1][1]
    ...
    def on_motion(self, event):
        ...
        if self.DRAW_METHOD == 3:
            ...
            if np.abs(np.linalg.norm((dx, dy))) > precision:
                ...
                x, y = event.xdata, event.ydata
                dv = x - xold, y - yold
                angle = self.get_ang(self.directions[-1], dv)  # Determination de l'angle de rotation
                # Ajout des commandes
    ...
    def Simulation(self):
        ...
        self._quit()
        import simulation_theorique.Simulation_theorique as simu, pygame
        pygame.init()
        window = simu.init_window()
        simu.main(window, "superpo")
    ...
    def add_command(self, c, i1, i2):
        ...
        if c == 'ROT' or c == 'LIN':
            ...
            self.textbox.insert('1.0',
                                "{0}({1}, {2})\n".format(c, round(self.command[-1][1], 2), round(self.command[-1][2])),
                                2)
    ...
    def addControlPt(self, x, y):
        ...
        if len(self.controlpoints) == 0:
            ...
            V = curve1.evaluate_multi(np.linspace(0.0, 1.0, 10))
            # print(V)
            X, Y = V[0, :], V[1, :]
            # print(X)
            # print(Y)
            ...
            xold, yold = self.plt_draw[-1][0], self.plt_draw[-1][1]
            ...
            dv = (self.plt_draw[-1][0] - self.plt_draw[-2][0], self.plt_draw[-1][1] - self.plt_draw[-2][0])
            self.add_direction(dv)
        self.update_graph()
